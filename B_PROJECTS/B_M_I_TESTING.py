        ...
        if saved_search_inputs_list[0] == 0:
            ...
            if os.path.isfile(saved_searches_file):
                ...
                for enumeration_number, searches in enumerate(saved_searches_list):
                    genres = searches.split(',')[0]
                    keywords = searches.split(',')[1]
                    print((str(enumeration_number) + ') '), '\n', '\n', 'GENRE: ', genres, '\n', 'KEYWORD(S): ',
                          keywords)

                saved_searches_sub_query()
            ...
            if os.path.isfile(saved_searches_file):
                ...
                for enumeration_number, searches in enumerate(saved_searches_list):
                    ...
                    search_term_removal_list.append([genres, keywords])

                separator_3()

                try:

                    search_term_to_remove = int(input('SELECT #: FOR SEARCH TERM TO REMOVE: '))

                    search_term_removal_list.remove(search_term_removal_list[search_term_to_remove])

                    with open(saved_searches_file, 'w', encoding='UTF-8', newline='') as f:
                        csv_writer = csv.writer(f)
                        for remaining_search_terms in search_term_removal_list:
                            csv_writer.writerow(remaining_search_terms)
